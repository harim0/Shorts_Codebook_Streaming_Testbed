...
from .coarse_sr_light import CoarseSRNet
from .decoder_light8 import DecoderLight
from .encoder8 import Encoder
...
class TexVQVAE(nn.Module):
    ...
    Pipeline:
        ...
        2. Encoder:   HR -> z (latent features at H/8, W/8)
        3. VQ:        z  -> z_q (quantized) + indices + losses
        4. DecoderLight:   (z_q, x_coarse) -> x_hat (reconstructed HR)
    """
    ...
    def forward(self, lr_prev, lr_curr, lr_next, hr_curr):
        ...
        lr_concat = torch.cat([lr_prev, lr_curr, lr_next], dim=1)
        x_coarse = self.coarse_sr(lr_concat)

        # 2. Encode HR texture -> latent at H/8, W/8
