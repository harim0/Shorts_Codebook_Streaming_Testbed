...
# ======== 환경/상수 ========
CHUNK_LENGTH = 4.0  # 초 단위 세그 길이
LOG_FILE = "/home/harim/Shorts_Codebook_Streaming_Testbed/abr-server/logs/qoe_out.log"
DNN_MODEL_ROOT = "/home/harim/Shorts_Codebook_Streaming_Testbed/cdn-server/contentServer/dash/model"
DNN_BUFFER_TH = 0.0   # 5s clips: trigger DNN on first ABR request

# ======== 유틸 ========
def clamp(x, lo, hi): return lo if x < lo else hi if x > hi else x

def parse_vid(url: str) -> str:
    # URL: .../dash/data/{class}/{vid}/multi_resolution.mpd
    # returns "{class}/{vid}" e.g. "Animal/SDR_Animal_23rp"
    try:
        parts = url.split('/')
...
def compute_dnn_total_bytes_for_vid(vid: str) -> float:
    # vid = "Animal/SDR_Animal_23rp" → class = "Animal"
    class_name = vid.split('/')[0]
    base_dir = os.path.join(DNN_MODEL_ROOT, class_name)
    total = 0
    try:
        for fn in ("decoder.pt", "coarse_sr.pt"):
...
def ensure_dnn_state_for_vid(input_dict, vid: str):
    st = input_dict['dnn_state'].get(vid)
    if st is not None:
        return st
    total = compute_dnn_total_bytes_for_vid(vid)
    if total <= 0:
        total = 1.0  # 0 나눗셈 방지
    st = {'total': total, 'remain': total, 'finished': False, 'step': 0}
    input_dict['dnn_state'][vid] = st
    return st
...
def make_handler(input_dict, logger, tp_estimator):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            dnn_mode = input_dict.get('dnn_mode', 0)
            if dnn_mode == 1 and vid:
                dnn_state = ensure_dnn_state_for_vid(input_dict, vid)

                ev = input_dict['events_record'][idx] if 0 <= idx < len(input_dict['events_record']) else {}
                buf_sec = float(ev.get('buffer', 0.0) or 0.0)
                sq      = validate_chunksizes(ev.get('chunksize_list'))
                Q       = len(sq[0]) if sq else 0    # 0..Q-1: video, Q: DNN

                if Q > 0 and not dnn_state['finished']:
                    if buf_sec >= DNN_BUFFER_TH and (dnn_state['step'] % 2 == 0):
                        return_code = Q 
                        # Logging 02.06
                        log_ev = copy.deepcopy(post)
                        log_ev["vid"] = vid
                        log_ev["chosen_quality"] = int(return_code)   # 0..Q-1 video, Q dnn
                        log_ev["event_type"] = "decision"
                        logger.append_download(log_ev)
                    dnn_state['step'] += 1
                print("[DNN-DEBUG] vid=", vid,
                    "buf_sec(post)=", post.get('buffer', None),
                    "buf_sec(ev)=", ev.get('buffer', None) if 'ev' in locals() else None,
                    "Q=", Q,
                    "step=", dnn_state['step'],
            ...
            if return_code != -2:
                print(f" ::: {parse_vid(post.get('url'))} ::: lastRequest: {post.get('lastRequest')}, "
...
def main():
    ...
    args = ap.parse_args()
    log_dir = f"/home/harim/Shorts_Codebook_Streaming_Testbed/abr-server/logs/{args.log}"
